...
4.1. Para VISÃO GERAL do sistema (sem filtrar por paciente), use `gerar_relatorio_telemetria(n_registros=N)`. Ex.: "como está o CardioMonitor em geral?", "qual o status da base de dados?", "resumo da telemetria dos últimos 50 batimentos do sistema". Retorna BPM médio/mín/máx agregados + percentual de irregulares de todo o dataset, NÃO filtra por paciente. Para análise de paciente específico, prefira `consultar_telemetria_dashboard`.
